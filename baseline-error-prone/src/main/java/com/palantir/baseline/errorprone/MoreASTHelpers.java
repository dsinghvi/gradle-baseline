...
final class MoreASTHelpers {
    ...
    static ImmutableSet<Type> getThrownCheckedExceptions(Tree tree, VisitorState state) {
        ImmutableSet<Type> allThrownExceptions = getThrownExceptions(tree, state);
        ImmutableSet.Builder<Type> builder = ImmutableSet.builderWithExpectedSize(allThrownExceptions.size());
        for (Type type : allThrownExceptions) {
            if (!state.getTypes().isSubtype(type, state.getTypeFromString(RuntimeException.class.getName()))
                    && !state.getTypes().isSubtype(type, state.getTypeFromString(Error.class.getName()))) {
                builder.add(type);
            }
        }
    ...
    private static final class ScanThrownTypes extends TreeScanner<Void, Void> {
        ...
        public Void visitThrow(ThrowTree tree, Void unused) {
            getThrownTypes().addAll(extractTypes(ASTHelpers.getType(tree.getExpression())));
